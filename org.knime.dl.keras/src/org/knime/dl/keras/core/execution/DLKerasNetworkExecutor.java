...
public class DLKerasNetworkExecutor implements DLNetworkExecutor<DLKerasExecutableNetwork> {
	...
	private void init(DLKerasExecutableNetwork network) throws IOException {
	 ...
	 */
	@Override
	public void execute(final DLKerasExecutableNetwork network, final Map<DLLayerDataSpec, DLLayerData[]> inputs,
			final Map<DLLayerDataSpec, DLLayerData[]> outputs, int batchSize) throws RuntimeException {
		...
		try {
			...
			m_kernel.putNetworkInput(inputs, batchSize);
