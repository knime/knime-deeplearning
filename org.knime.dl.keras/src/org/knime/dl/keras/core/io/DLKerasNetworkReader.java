 ...
 */
package org.knime.dl.keras.core.io;
...
/**
 * Reads in a {@link DLKerasNetwork Keras network} from a URL.
 *
 * @author Marcel Wiedenmann, KNIME, Konstanz, Germany
 * @author Christian Dietz, KNIME, Konstanz, Germany
 */
public class DLKerasNetworkReader implements DLNetworkReader<DLKerasNetwork> {

    private final DLKerasBackend m_backend;

    /**
     * Creates a new network reader for the given Keras back end.
     *
     * @param backend the Keras back end
     */
    public DLKerasNetworkReader(final DLKerasBackend backend) {
        m_backend = backend;
    }
     ...
     */
    @Override
    public DLKerasNetwork readNetwork(final URL source) throws IllegalArgumentException, IOException {
        ...
        try {
            ...
            throw new IOException("Connection to Keras could not be established.", e);
        }
        try {
            final String fileExtension = FilenameUtils.getExtension(filePath);
            final DLKerasNetworkHandle network;
            if (fileExtension.equals("h5")) {
                network = kernel.loadNetworkFromH5(filePath);
            } else if (fileExtension.equals("json")) {
                network = kernel.loadNetworkSpecFromJson(filePath);
            } else if (fileExtension.equals("yaml")) {
                network = kernel.loadNetworkSpecFromYaml(filePath);
            } else {
                throw new IllegalArgumentException(
                    "Keras network reader only supports files of type h5, json and yaml.");
            }
            final DLKerasNetworkSpec networkSpec = kernel.extractNetworkSpec(network, m_backend.getTypeMap());
            return new DLKerasNetwork(networkSpec, source);
        } catch (final Exception e) {
