 ...
 */
public class DefaultNestedMemberReadAccess<T, S> extends DefaultMemberReadAccess<T, S>
    implements NestedMemberReadAccess<T, S> {

    private NestedValueReadAccess<T, S> m_access;

    /**
     * @param member
     * @param readAccess
     */
    public DefaultNestedMemberReadAccess(Member<T> member, NestedValueReadAccess<T, S> readAccess) {
        super(member, readAccess);
        m_access = readAccess;
    }
    ...
    public StructInstance<? extends MemberReadInstance<?>, ?> getStructInstance(S storage)
        ...
        return m_access.getStructInstance(storage);
    }

    @Override
    public Class<T> getType(S storage) throws InvalidSettingsException {
        return m_access.getType(storage);
    }
