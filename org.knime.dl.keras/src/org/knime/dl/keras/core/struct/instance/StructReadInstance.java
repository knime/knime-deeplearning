 ...
 */
class StructReadInstance<S> implements StructInstance<MemberReadInstance<?>, S> {

    private S m_storage;

    private StructAccess<? extends MemberReadAccess<?, S>> m_access;

    public StructReadInstance(S storage, StructAccess<? extends MemberReadAccess<?, S>> access) {
        m_storage = storage;
        m_access = access;
    }

    @Override
    public MemberReadInstance<?> member(String key) {
        return createMemberReadInstance(m_access.member(key), m_storage);
    }

    @Override
    public Iterator<MemberReadInstance<?>> iterator() {
        final Iterator<? extends MemberReadAccess<?, S>> del = m_access.members().iterator();

        return new Iterator<MemberReadInstance<?>>() {
            ...
            @Override
            public MemberReadInstance<?> next() {
                return createMemberReadInstance(del.next(), m_storage);
            }
        };
    }
