 ...
 */
class NodeSettingsPrimitiveArrayAccessRO<T> extends AbstractNodeSettingsReadAccess<T> {

    public NodeSettingsPrimitiveArrayAccessRO(Member<T> member) {
        super(member);
    }

    @Override
    public T get(NodeSettingsRO storage, String key) throws InvalidSettingsException {
        final Class<?> rawType = ClassUtils.primitiveToWrapper(m_member.getRawType().getComponentType());
        final Object value;
        if (Boolean.class.equals(rawType)) {
            value = storage.getBooleanArray(key);
        } else if (Double.class.equals(rawType)) {
            value = storage.getDoubleArray(key);
        } else if (Float.class.equals(rawType)) {
            value = storage.getFloatArray(key);
        } else if (Byte.class.equals(rawType)) {
            value = storage.getByteArray(key);
        } else if (Short.class.equals(rawType)) {
            value = storage.getShortArray(key);
        } else if (Integer.class.equals(rawType)) {
            value = storage.getIntArray(key);
        } else if (Long.class.equals(rawType)) {
            value = storage.getLongArray(key);
        } else {
