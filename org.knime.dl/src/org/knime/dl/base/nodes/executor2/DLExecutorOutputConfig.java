...
final class DLExecutorOutputConfig extends DLAbstractIOConfig<DLExecutorGeneralConfig> {
    ...
    DLExecutorOutputConfig(final DLTensorId outputTensorId, final String outputTensorName,
        ...
        put(new SettingsModelConfigEntry<>(CFG_KEY_CONVERTER, DLTensorToDataCellConverterFactory.class,
            s -> new SettingsModelStringArray(s, null), this::entryToSettingsModel, this::settingsModelToValue));
    ...
    private DLTensorToDataCellConverterFactory<?, ?> settingsModelToValue(final SettingsModelStringArray sm)
        throws InvalidSettingsException {
        final String[] array = sm.getStringArrayValue();
        return DLTensorToDataCellConverterRegistry.getInstance().getConverterFactory(array[1]).orElseThrow(
            () -> new InvalidSettingsException("Converter '" + array[0] + " (" + array[1] + ") for network output '"
