            ...
            public void loadSettingsFrom(final NodeSettingsRO settings) throws InvalidSettingsException {
                ...
                if (newExecCtx[1] != null) {
                    m_value = DLExecutionContextRegistry.getInstance().getExecutionContext(newExecCtx[1]).orElseThrow(
                        () -> new InvalidSettingsException("Executor back end '" + newExecCtx[0] + "' (" + newExecCtx[1]
                            + ") could not be found. Are you missing a KNIME Deep Learning extension?"));
                }
            }

        });
        put(SettingsModelConfigEntries.createIntegerBoundedConfigEntry(CFG_KEY_BATCH_SIZE, defaultBatchSize, 1,
            Integer.MAX_VALUE));
