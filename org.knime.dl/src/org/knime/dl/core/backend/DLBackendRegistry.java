...
public final class DLBackendRegistry {
	...
	/**
	 * Returns the {@link DLBackend back end} that corresponds to the given identifier if present.
	 *
	...
	/**
	 * Returns the preferred {@link DLBackend back end} for the given {@link DLProfile profile} if present.
	 *
	 * @param profile the profile
	 * @return the preferred back end
	 * @throws NullPointerException if the argument is null
	 */
	public static Optional<DLBackend> getPreferredBackend(final DLProfile profile) throws NullPointerException {
		checkNotNull(profile);
		if (profile.size() == 0) {
			return Optional.empty();
		}
		if (profile.size() == 1) {
			return Optional.of(profile.iterator().next());
		}
		// TODO
		return Optional.of(profile.iterator().next());
	}
