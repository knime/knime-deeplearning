...
class DLPythonPredictorNodeDialog extends DataAwareNodeDialogPane {
	...
	protected DLPythonPredictorNodeDialog() {
		m_sourceCodePanel = new PythonSourceCodePanel(DLPythonPredictorNodeConfig.getVariableNames(),
	...
	protected void saveSettingsTo(NodeSettingsWO settings) throws InvalidSettingsException {
		DLPythonPredictorNodeConfig config = new DLPythonPredictorNodeConfig();
	...
	protected void loadSettingsFrom(NodeSettingsRO settings, PortObject[] input) throws NotConfigurableException {
		...
		DLNetworkPortObject portObject = (DLNetworkPortObject) input[0];
		if (portObject != null && portObject.getNetwork() instanceof DLPythonLoadable) {
			m_sourceCodePanel.registerWorkspacePreparer(new WorkspacePreparer() {
				@Override
				public void prepareWorkspace(PythonKernel kernel) {
					try {
						String name = ((DLPythonLoadable) portObject.getNetwork()).load(kernel);
