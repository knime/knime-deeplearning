 ...
 */
public class DLPythonBitBufferSerializerFactory extends SerializerFactory<DLPythonBitBuffer>
    implements DLSerializerFactory {

    /**
     * The unique identifier of this serializer factory.
     */
    public static final String IDENTIFIER = "org.knime.dl.python.core.data.serde.DLPythonBitBufferSerializerFactory";

    /**
     * Empty framework constructor.
     */
    public DLPythonBitBufferSerializerFactory() {
        super(DLPythonBitBuffer.class);
    }

    @Override
    public Serializer<? extends DLPythonBitBuffer> createSerializer() {
        ...
        return value -> {
            ...
            final long numBytes = size;
            if (numBytes > Integer.MAX_VALUE) {
                throw new IOException(
                    "Transmitting data to Python failed. Buffer size exceeds the limit of 2^31-1 bytes.");
            }
            // Create the target array
            final byte[] target = new byte[(int)numBytes];
            value.readToByteArray(target, 0, (int)size);
            // TODO: we serialize to flat buffers for now
