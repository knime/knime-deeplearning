 ...
 */
public class DLPythonFloatBufferDeserializerFactory extends DeserializerFactory implements DLPythonDeserializerFactory {
	...
	public Deserializer createDeserializer() {
		return new DLPythonDeserializer<DLPythonFloatBuffer>() {
			...
			public DataCell deserialize(final byte[] bytes, final FileStoreFactory fileStoreFactory)
				...
				final DLPythonFloatBuffer value = new DLPythonFloatBuffer(buffer.capacity() / Float.BYTES);
				floatBuffer.get(value.getStorage());
				return value;
			}

			@Override
			public void deserialize(final byte[] bytes, final DLLayerData<DLPythonFloatBuffer> data) {
				final ByteBuffer buffer = ByteBuffer.wrap(bytes);
				buffer.order(ByteOrder.LITTLE_ENDIAN);
				// TODO: we serialize to a flat buffer for now
				// final int numDimensions = buffer.getInt();
				...
				final FloatBuffer floatBuffer = buffer.asFloatBuffer();
				floatBuffer.get(data.getData().getStorage());
			}
		};
	}

	@Override
	public Class<? extends DLPythonFloatBuffer> getBufferType() {
