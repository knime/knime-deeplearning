 ...
 */
public class DLPythonShortBufferSerializerFactory extends SerializerFactory<DLPythonShortBuffer>
    implements DLSerializerFactory {

    /**
     * The unique identifier of this serializer factory.
     */
    public static final String IDENTIFIER = "org.knime.dl.python.core.data.serde.DLPythonShortBufferSerializerFactory";

    /**
     * Empty framework constructor.
     */
    public DLPythonShortBufferSerializerFactory() {
        super(DLPythonShortBuffer.class);
    }

    @Override
    public Serializer<? extends DLPythonShortBuffer> createSerializer() {
        ...
        return value -> {
            ...
            final short[] tensorStorage = value.getStorageForReading(value.getNextReadPosition(), size);
            final ByteBuffer buffer = ByteBuffer.allocate((int)numBytes);
            buffer.order(ByteOrder.LITTLE_ENDIAN);
            buffer.asShortBuffer().put(tensorStorage, (int)value.getNextReadPosition(), (int)size);
            // TODO: we serialize to flat buffers for now
