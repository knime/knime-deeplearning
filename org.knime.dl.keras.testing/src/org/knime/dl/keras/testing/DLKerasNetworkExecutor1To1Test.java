...
public class DLKerasNetworkExecutor1To1Test {
    ...
    public void test() throws IOException, InvalidPathException, MalformedURLException {
        ...
        for (final DLLayerDataSpec inputSpec : execSpec.getInputSpecs()) {
            final DLLayerData<?> input = execNetwork.getInputForSpec(inputSpec);
            populate(input);
        }
        final Set<DLLayerDataSpec> selectedOutputs = Collections.singleton(execSpec.getOutputSpecs()[0]);

        execNetwork.execute(selectedOutputs, new Consumer<Map<DLLayerDataSpec, DLLayerData<?>>>() {

            @Override
            public void accept(final Map<DLLayerDataSpec, DLLayerData<?>> t) {
                // TODO
            }
        });
    }
    ...
    private static void populate(final DLLayerData<?> data) {
        if (data.getData() instanceof DLWritableFloatBuffer) {
            final DLWritableFloatBuffer buffer = (DLWritableFloatBuffer)data.getData();
            buffer.resetWrite();
